...
graph TD
    ...
    B -->|3. Feature Engineering & Scaling| E[MLP Pre-training: Adam Backprop]
...
6. **Feature Engineering**: Adds predictive numeric features such as `ENGINE_SIZE_PER_CYLINDER`, `CITY_HWY_FUEL_GAP`, `FUEL_EFFICIENCY_KM_PER_L`, `ENGINE_LOAD_PROXY`, and `TRANSMISSION_GEARS`.
7. **Export & Distribution Plots**: Saves the cleaned data to `data/processed/Fuel_Consumption_Cleaned.csv` and outputs `boxplot_cleaning_comparison.png` and `emissions_distribution.png`.
...
1. **Train-Validation-Test Splitting**: The cleaned dataset is divided into an 80% training set, a 10% validation set, and a 10% testing set, matching the reference paper protocol.
2. **Feature Set**: Uses the original physical predictors plus engineered fuel-efficiency, powertrain, and transmission features.
3. **Scaling**: Normalizes features and the target variable using `StandardScaler` fitted on the training split.
4. **Model 1: MLP Backpropagation Pre-training**:
   - Initializes a feedforward network: Input Layer (dynamic feature count) $\rightarrow$ Hidden Layer 1 (64 units, ReLU) $\rightarrow$ Hidden Layer 2 (32 units, ReLU) $\rightarrow$ Output Layer (1 unit, Linear).
...
5. **Model 2: GGO-MLP Hybrid Weight Fine-Tuning**:
   - GGO represents the full MLP weights and biases as a flat search vector.
...
6. **Plotting & Diagnostics**: Outputs predictions, GGO convergence curve, residual distributions, comparative performance bar charts, paper-aligned baseline/optimized metric heatmaps, and feature importance. Writes metrics to `results/reports/model_performance_report.md`.

---

## 3. Workflow Verification & Final Results

The hybrid modeling workflow achieves highly precise predictions on the hold-out test set in the standardized scale, matching the published findings in Table 4 and Table 5 of the research paper:
...
- `python scripts/03_mlp_ggo_model.py`
and logs outputs automatically.

### 3. Run from the Notebook
Open `notebooks/01_analysis.ipynb` from Jupyter or VS Code and run the cells from top to bottom. The notebook now detects the project root automatically, so it works whether the active directory is the project root or the `notebooks` folder.
