...
def run_pipeline():
    ...
    bp_vector = mlp.get_weights_vector()
    D_size = len(bp_vector)
    
    pop_size = 30
    ggo_iterations = 50
    
    # Initialize GGO population around pre-trained backprop weights
    X_ggo = np.zeros((pop_size, D_size))
    X_ggo[0] = bp_vector
    for i in range(1, pop_size):
        X_ggo[i] = bp_vector + np.random.normal(0, 0.015, size=D_size)
        
    fitness = np.zeros(pop_size)
    ggo_mlp = NumPyMLP()
    
    # Initial fitness evaluation (Validation MSE)
    for i in range(pop_size):
        ggo_mlp.set_weights_vector(X_ggo[i])
        val_preds = ggo_mlp.forward(X_val_scaled)
        fitness[i] = np.mean((val_preds - y_val_scaled.reshape(-1, 1)) ** 2)
        
    best_idx = np.argmin(fitness)
    leader_pos = X_ggo[best_idx].copy()
    leader_fit = fitness[best_idx]
    print(f"Initial best GGO fitness (Val MSE): {leader_fit:.6f}")
    
    ggo_history = [leader_fit]
    
    # GGO Metaheuristic Loop
    for t in range(1, ggo_iterations + 1):
        a = 2.0 - 2.0 * (t / ggo_iterations) # control parameter decreasing to 0
        X_new = X_ggo.copy()
        
        # Exploration Phase (first 15 agents)
        for i in range(15):
            if t % 2 == 0:
                r3 = np.random.rand()
                if r3 < 0.5:
                    ...
                    if abs(A) < 1.0:
                        r2 = np.random.rand()
                        C = 2.0 * r2
                        X_new[i] = leader_pos - A * np.abs(C * leader_pos - X_ggo[i])
                    else:
                        # 3 random paddles
                        idx_p = np.random.choice(pop_size, 3, replace=False)
                        X_p1, X_p2, X_p3 = X_ggo[idx_p[0]], X_ggo[idx_p[1]], X_ggo[idx_p[2]]
                        z = 1.0 - (t / ggo_iterations)**2
                        w1 = np.random.uniform(0.5, 1.5)
                        w2 = np.random.uniform(0.5, 1.5)
                        w3 = np.random.uniform(0.5, 1.5)
                        X_new[i] = w1 * X_p1 + z * w2 * (X_p2 - X_p3) + (1.0 - z) * w3 * (X_ggo[i] - X_p1)
                else:
                    # Stochastic diversification
                    w4 = np.random.uniform(0.5, 1.5)
                    w1 = np.random.uniform(0.5, 1.5)
                    b = 1.0
                    l = np.random.uniform(-1, 1)
                    r4 = np.random.rand()
                    r5 = np.random.rand()
                    X_new[i] = w4 * np.abs(leader_pos - X_ggo[i]) * np.exp(b * l) * np.cos(2.0 * np.pi * l) + 2.0 * w1 * (r4 + r5) * leader_pos
            else:
                # Update towards flock
                D = 0.5
                w = 0.5
                z = 1.0 - (t / ggo_iterations)**2
                X_flock = X_ggo[np.random.choice(pop_size)]
                X_new[i] = X_ggo[i] + D * (1.0 + z) * w * (X_ggo[i] - X_flock)
                
        # Exploitation Phase (last 15 agents)
        for j in range(15, pop_size):
            if t % 2 == 0:
                # 3 designated sentries
                idx_s = np.random.choice(pop_size, 3, replace=False)
                X_s1, X_s2, X_s3 = X_ggo[idx_s[0]], X_ggo[idx_s[1]], X_ggo[idx_s[2]]
                
                r1 = np.random.rand()
                A1 = 2.0 * a * r1 - a
                r2 = np.random.rand()
                C1 = 2.0 * r2
                X1 = X_s1 - A1 * np.abs(C1 * X_s1 - X_ggo[j])
                
                r1 = np.random.rand()
                A2 = 2.0 * a * r1 - a
                r2 = np.random.rand()
                C2 = 2.0 * r2
                X2 = X_s2 - A2 * np.abs(C2 * X_s2 - X_ggo[j])
                
                r1 = np.random.rand()
                A3 = 2.0 * a * r1 - a
                r2 = np.random.rand()
                C3 = 2.0 * r2
                X3 = X_s3 - A3 * np.abs(C3 * X_s3 - X_ggo[j])
                ...
                z = 1.0 - (t / ggo_iterations)**2
                X_flock = X_ggo[np.random.choice(pop_size)]
                X_new[j] = X_ggo[j] + D * (1.0 + z) * w * (X_ggo[j] - X_flock)
                
        # Constrain weights
        X_new = np.clip(X_new, -3.0, 3.0)
        
        # Evaluate fitness
        for i in range(pop_size):
            ggo_mlp.set_weights_vector(X_new[i])
            val_preds = ggo_mlp.forward(X_val_scaled)
            fit_val = np.mean((val_preds - y_val_scaled.reshape(-1, 1)) ** 2)
            fitness[i] = fit_val
            
            # Leader update
            if fit_val < leader_fit:
                leader_fit = fit_val
                leader_pos = X_new[i].copy()
                
        X_ggo = X_new.copy()
        ggo_history.append(leader_fit)
        
        if t % 10 == 0 or t == ggo_iterations:
            print(f"Iteration {t}/{ggo_iterations} - Best GGO Fitness (Val MSE): {leader_fit:.7f}")
            
    # Save optimized weights to GGO model
    ggo_mlp.set_weights_vector(leader_pos)
    ...
    ax.set_ylabel('Fitness (MSE on validation)', fontsize=12)
    plt.tight_layout()
    ...
    # --- EVALUATE ON TEST SET & MAP TO ORIGINAL SCALE ---
    # Standard MLP Predictions
    mlp_preds_scaled = mlp.forward(X_test_scaled)
    mlp_preds_orig = scaler_y.inverse_transform(mlp_preds_scaled).flatten()
    
    # GGO MLP Predictions
    ggo_preds_scaled = ggo_mlp.forward(X_test_scaled)
    ggo_preds_orig = scaler_y.inverse_transform(ggo_preds_scaled).flatten()
    ...
    metrics = {
        'MLP': {
            'MSE': mean_squared_error(y_test_orig, mlp_preds_orig),
            'RMSE': np.sqrt(mean_squared_error(y_test_orig, mlp_preds_orig)),
            'MAE': mean_absolute_error(y_test_orig, mlp_preds_orig),
            'R2': r2_score(y_test_orig, mlp_preds_orig)
        },
        'GGO-MLP': {
            'MSE': mean_squared_error(y_test_orig, ggo_preds_orig),
            'RMSE': np.sqrt(mean_squared_error(y_test_orig, ggo_preds_orig)),
            'MAE': mean_absolute_error(y_test_orig, ggo_preds_orig),
            'R2': r2_score(y_test_orig, ggo_preds_orig)
        }
    }
    
    print("\nFinal Test Metrics Comparison (Original Scale - g/km):")
    ...
    for m in ['MSE', 'RMSE', 'MAE', 'R2']:
        ...
        if m == 'R2':
            ...
            imp = ((v1 - v2) / v1) * 100
        print(f"{m:<6} | {v1:<12.5f} | {v2:<12.5f} | {imp:<12.3f}%")
    ...
    plt.suptitle('Predictions vs Actual CO2 Emissions Comparison', fontsize=15, fontweight='bold', y=0.98)
    plt.tight_layout()
    ...
    mlp_res = y_test_orig - mlp_preds_orig
    ggo_res = y_test_orig - ggo_preds_orig
    
    sns.histplot(mlp_res, kde=True, bins=35, color='#e17055', edgecolor='white', alpha=0.8, ax=axes[0])
    axes[0].set_title('Standard MLP Prediction Errors (Residuals)', fontsize=12, fontweight='bold')
    axes[0].set_xlabel('Error (Actual - Predicted) (g/km)', fontsize=11)
    axes[0].set_ylabel('Count', fontsize=11)
    axes[0].set_facecolor('#ffffff')
    
    sns.histplot(ggo_res, kde=True, bins=35, color='#20bf6b', edgecolor='white', alpha=0.8, ax=axes[1])
    axes[1].set_title('GGO-MLP Prediction Errors (Residuals)', fontsize=12, fontweight='bold')
    axes[1].set_xlabel('Error (Actual - Predicted) (g/km)', fontsize=11)
    axes[1].set_facecolor('#ffffff')
    ...
    baseline_mse = mean_squared_error(y_test_orig, ggo_preds_orig)
    ...
    for idx, col in enumerate(feature_names):
        ...
        # Shuffle columns
        X_temp[:, idx] = np.random.permutation(X_temp[:, idx])
        
        shuffled_preds_scaled = ggo_mlp.forward(X_temp)
        shuffled_preds_orig = scaler_y.inverse_transform(shuffled_preds_scaled).flatten()
        
        shuffled_mse = mean_squared_error(y_test_orig, shuffled_preds_orig)
...
|-------------------|--------------------------|----------------|-------------------|
| **MSE** (g²/km²)  | {metrics['MLP']['MSE']:.4f} | {metrics['GGO-MLP']['MSE']:.4f} | {((metrics['MLP']['MSE'] - metrics['GGO-MLP']['MSE']) / metrics['MLP']['MSE']) * 100:.3f}% |
| **RMSE** (g/km)   | {metrics['MLP']['RMSE']:.4f} | {metrics['GGO-MLP']['RMSE']:.4f} | {((metrics['MLP']['RMSE'] - metrics['GGO-MLP']['RMSE']) / metrics['MLP']['RMSE']) * 100:.3f}% |
| **MAE** (g/km)    | {metrics['MLP']['MAE']:.4f} | {metrics['GGO-MLP']['MAE']:.4f} | {((metrics['MLP']['MAE'] - metrics['GGO-MLP']['MAE']) / metrics['MLP']['MAE']) * 100:.3f}% |
| **R² Score**      | {metrics['MLP']['R2']:.4f} | {metrics['GGO-MLP']['R2']:.4f} | {((metrics['GGO-MLP']['R2'] - metrics['MLP']['R2']) / (1 - metrics['MLP']['R2'])) * 100:.3f}% (of unexplained variance) |
...
2. **Predictive Capability**: Both models achieve exceptional performance, explaining over **{metrics['GGO-MLP']['R2']*100:.2f}%** of the variation in vehicle CO2 emissions using only 3 simple vehicle characteristics.
